# music_suggestions

This is an in progress flask app, the finished product 
will be a website that users can recommend songs to each other 
and keep track of songs that they have listened to.

A Svelte-kit app with server side rendering is acting as a
front-end for this flask app and at some point caching will
be happening on this frontend server as well.



